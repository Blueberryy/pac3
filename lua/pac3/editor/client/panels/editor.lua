...
function PANEL:Init()
	...
	self.zoomframe:SetSize( 180, 150 )
		...
		local SETTING_MARGIN_TOP = 6
			...
			self.persistcheckbox:SetText("#pac_persistent_fov")
			...
			self.persistlabel:SetText("#pac_persistent_fov_tip")
			...
			self.mwheelcheckbox:SetText("#pac_mouse_wheel")
			...
			self.smoothcheckbox:SetText("#pac_smooth_zooming")
			...
			self.smoothlabel:SetText("#pac_smooth_zooming_tip")
		...
		self.sliderpanel:Dock(TOP)
			...
			self.zoomslider:SetText("#pac_camera_fov")
...
local function drawTimeBox(text, time, x, y)
	local str = string.format(L"%s: %.3f ms", L(text), time)
...
function PANEL:PaintOver(w, h)
	...
	surface.SetTextColor(textCol)
	cam.IgnoreZ(true)
	local str = string.format(L"%s: %.3f ms", L("average render time"), renderTime * 1000)
